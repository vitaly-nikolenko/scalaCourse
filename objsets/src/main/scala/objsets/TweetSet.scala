...
abstract class TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    ...
    new Cons(mR, if (remove(mR).isEmpty) Nil else remove(mR).descendingByRetweet)
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    if (left.isEmpty && right.isEmpty) elem
      ...
      (right.isEmpty || elem.retweets >= right.mostRetweeted.retweets)) elem
    else if (left.isEmpty || (!right.isEmpty && left.mostRetweeted.retweets <= right.mostRetweeted.retweets)) right.mostRetweeted
